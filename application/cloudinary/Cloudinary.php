...
class Cloudinary
{
    ...
    public static function generate_transformation_string(&$options = array())
    {
        ...
        $param_filter = Cloudinary::param_filter($value);
        ...
        $join_pair = Cloudinary::join_pair($key, $value);
	...
	public static function join_pair($key, $value) {
		return $key . "_" . $value;
	}
	
	public static function param_filter($value) {
    ...
    public static function cloudinary_url($source, &$options = array())
    {
        ...
        if ($sign_url && $auth_token) {
            ...
            $token = AuthToken::generate(array_merge($auth_token, array("url" => $path)));
            $source = $source . "?" . $token;
        }
    ...
    public static function generate_auth_token($options)
    {
        ...
        return AuthToken::generate($token_options);
    }
    ...
    public static function build_archive_params(&$options)
    {
        ...
        array_walk(
            $params,
            Cloudinary::key_value($value, $key)
        );
	...
	public static function key_value(&$value, $key) {
    ...
    public static function html_attrs($options, $only = null)
    {
        ...
        $join_pair = Cloudinary::join_pair2($key, $value);

        return implode(" ", array_map($join_pair, array_keys($attrs), array_values($attrs)));
    }
	
	public static function join_pair2($key, $value) {
		$out = $key;
        if (!empty($value)) {
